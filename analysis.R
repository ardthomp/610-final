...
# -------------------FIGURE 4: LINEAR REGRESSION SUMMARY TABLE---------------
...
reg_table <- tibble(
  Statistic = c("Slope (β₁)", "Intercept (β₀)", "Adjusted R²", "Residual SD"),
  Value = c("1.18***", "48.5", "0.901", "1013"),
  Interpretation = c(
    "Forecast underpredicts during high-volume weeks (β > 1)",
    "Small baseline offset",
    "Explains ~90% of observed variation",
    "Typical magnitude of forecast error"
  )
)
...
tab <- reg_table %>%
  gt() %>%
  tab_header(
    title = md("**Linear Regression Summary**"),
    subtitle = md("Point Forecast Accuracy")
  ) %>%
  tab_style(
    style = cell_text(font = "inter", weight = "bold", color = "#225739"),
